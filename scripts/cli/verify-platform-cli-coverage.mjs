...
export function inspectCliExecutionPolicy(
  source,
  sourceName = "src/commands/executor.ts",
) {
...
function cliInvocation() {
  ...
  try {
    if (statSync(tsx).isFile()) {
      return {
        command: tsx,
        prefix: [join(repositoryRoot, "src", "entry.ts")],
      };
    }
  } catch {
  ...
  return {
    ...
    prefix: [join(repositoryRoot, "dist", "entry.js")],
  };
}
...
export async function runPlatformCliCoverageGate() {
  const platformRoot = process.env.LUNA_PLATFORM_ROOT
    ? resolve(repositoryRoot, process.env.LUNA_PLATFORM_ROOT)
    : defaultPlatformRoots.find(root =>
        statSync(join(root, "internal", "api", "router.go"), {
          throwIfNoEntry: false,
        })?.isFile(),
      );
  if (!platformRoot) {
    throw new Error(
      "Luna DevOps source checkout was not found. Set LUNA_PLATFORM_ROOT to its directory.",
    );
  }
  const routerPath = join(platformRoot, "internal", "api", "router.go");
  const openApiPath = join(platformRoot, "openapi", "openapi.yaml");
