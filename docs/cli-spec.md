...
本方案没有依赖无法实现的技术能力，但它不是一个只在 CLI 命令目录中补充入口即可完成的任务。第一版要达到“全部平台 API 可调用且 95% 以上可用”，必须同步完成服务端契约、认证和协议能力改造。
...
在现有“面向人类的 CLI”基础上增加 `output=json`，仍不足以形成可靠的 Agent 工具。对照 GitHub CLI、Terraform、OpenAI、Anthropic、OWASP、OpenAPI Arazzo、JSON Schema 和 RFC 9457 后，Luna CLI 还需要补齐以下边界：
...
CLI、共享 TypeScript API Client、API 契约快照和配套 Skill 统一维护在
`LiteyukiStudio/luna-cli` 独立仓库中。Luna DevOps 平台仓库不通过 submodule、
subtree 或 subrepo 关联 CLI，只保留被 Git 忽略的 `cli/` 本地联调目录。
...
- `src`：提供 Node/Bun Transport、Bearer Token、本地凭据、终端 MFA、命令注册与输出适配器。
- `openapi`：保存平台 API 契约快照；CI 从平台仓库拉取最新契约并检查漂移。
- `skills/luna-devops`：与 CLI 同版本发布的渐进式 Skill，领域资料按需加载。
- Luna DevOps 平台仓库继续独立维护 Go API 和 Web 前端，不从 CLI 仓库反向导入 TypeScript 包。
...
- Secret、Token、密码、恢复码和其他密钥材料不允许使用内联 `key=value`，只能使用安全提示、`@-` 或权限合规的文件。
...
`dryRun=client` 只验证本地 Schema 并输出标准化请求预览，不能声称服务端接受该请求。`dryRun=server` 对应权威的服务端校验；每个 operation 必须声明支持 `none`、`client`、`server` 或 `both`。高风险变更不能用 client dry-run 替代服务端 plan。
...
```json
{
  ...
  "warnings": [],
  "requiredScopes": [],
  "mfaPurpose": "runtime_cluster_update",
...
```bash
luna auth mfa-verify purpose=runtime_terminal
```
...
- `message` 仅用于保留后端或第三方诊断，不要求本地化，不得包含堆栈、Secret、Token、SQL、凭据内容或内部网络敏感信息。
...
仓库中的 [`skills`](../skills) 是 CLI 的薄编排层，调用链固定为：
...
curl -fL -o luna "https://github.com/LiteyukiStudio/luna-cli/releases/download/${version}/${asset}"
curl -fL -o SHA256SUMS "https://github.com/LiteyukiStudio/luna-cli/releases/download/${version}/SHA256SUMS"
...
- `repository.url` 必须与 npm Trusted Publisher 配置的 GitHub 仓库完全一致。
- 仓库中的 `version` 固定为 `0.0.0-development`，只表示源码开发态，不参与正式版本决策；源码清单使用 `private: true` 阻止绕过发布工作流直接发布。
- `v*` Git tag 是发布版本的唯一来源。工作流去掉 `v` 后校验 SemVer，在临时 npm 打包目录移除 `private` 标记并写入发布版本，同时在构建时把同一版本注入 npm JavaScript 制品和 Bun 二进制；不得修改或提交源工作树中的版本。
...
平台与 CLI 位于不同仓库，各自使用标准
`v<major>.<minor>.<patch>[-prerelease]` tag。CLI 仓库中的 `v*` 只触发 CLI 和
配套 Skill 发版，不会触发 Luna DevOps 平台发版。
...
手动重跑只能选择 CLI 仓库中已经存在且符合 `v<semver>` 的 tag。
...
| 工作流 | 触发 | 职责 |
| --- | --- | --- |
| `.github/workflows/cli-ci.yml` | PR、`main` push、手动 | 类型、Lint、测试、平台契约、npm tarball 和 Bun 编译 smoke |
| `.github/workflows/cli-release.yml` | `v*` tag、受限手动重跑 | 版本校验、完整构建、签名、npm OIDC 发布、GitHub Release |
| `.github/workflows/cli-skills-release.yml` | 手动 | 对指定 CLI tag 重跑配套 Skill 验证和打包 |
...
1. 使用冻结锁文件安装依赖；
2. 校验发布辅助脚本和配套 Skill；
3. 只读 checkout `LiteyukiStudio/luna-devops` 到 `.upstream/luna-devops`；
4. 对比 `openapi/openapi.yaml` 与平台仓库 OpenAPI；
5. 使用 `LUNA_PLATFORM_ROOT` 运行平台 API 命令覆盖率检查；
6. 运行 TypeScript 类型检查、ESLint、Vitest 和构建；
7. 对真实 npm tarball 执行 npm/pnpm 全局安装 smoke；
8. 在原生 runner 上构建并执行当前平台的 Bun 二进制。
...
### 15.7 `cli-release.yml` 发布流程

发布工作流以 tag 为唯一版本来源，依次执行：

1. 校验 tag、SemVer 和当前提交；
2. 重跑与 CI 相同的平台契约、Skill、类型、Lint、测试门禁；
3. 并行构建并在原生 runner smoke Linux/macOS 二进制；
...
- `release-metadata.mjs` 先校验 tag 符合 `v<semver>`，再去掉 `v` 前缀并生成发布版本、预发布标识和 npm dist-tag；
- `pack-npm.mjs` 只在临时目录把 tag 版本写入发布清单，校验 `npm pack` 返回版本和文件白名单，源 `package.json` 始终保留开发占位版本；
...
Windows 与 Alpine/musl 不进入二进制 Job，发布与 smoke 均复用已经过 npm/pnpm 全局安装验证的 Node.js 制品。交叉编译只能证明生成成功，不能替代目标运行时验证；原生 runner 不可用的平台不应被加入已支持的二进制矩阵。
...
5. 创建一个指向目标提交、且版本尚未发布的 `v*` tag 验证 OIDC 发布。无需修改 `package.json`；不能复用首次手动发布的 tag 版本做认证验收，因为幂等发布逻辑发现远端内容一致时会跳过 `npm publish`。
...
22. 普通 `v*` 只触发平台项目发版，`v*` 只触发 CLI 发版；两套工作流都拒绝对方的 tag 命名空间。
