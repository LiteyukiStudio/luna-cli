  ...
  'en-US': {
    translation: {
      ...
      categories: {
        ...
        'kubeconfig': 'Secure kubectl configuration files',
      ...
      parameters: {
        ...
        context: 'Stable projectId:runtimeClusterId[:applicationId] selection; repeat for multiple contexts.',
        credential: 'Saved credential name.',
        credentialName: 'Display name for the short-lived kube credential.',
        cursor: 'Pagination cursor returned by the previous request.',
        destination: 'Target kubeconfig path; write requires a new file, while merge preserves existing entries.',
        expiresInDays: 'Credential lifetime in days; allowed values are 1, 7, or 30.',
        kubeScope: 'kubectl capability: read, write, or connect; repeat for multiple capabilities.',
        ...
        replaceConflicts: 'Replace only same-name cluster, user, and context entries whose content differs.',
      ...
      commands: {
        ...
        kubeconfig: {
          merge: { summary: 'Create a short-lived credential and merge it into one kubeconfig safely.' },
          write: { summary: 'Create a short-lived credential and write a new kubeconfig safely.' },
        },
        project: {
      ...
      errors: {
        ...
        dry_run_unsupported: 'kubeconfig commands do not support dry-run.',
        invalid_arguments: 'Input validation failed.',
        kubeconfig_conflict: 'The target kubeconfig contains conflicting same-name entries. Review them or explicitly set replaceConflicts=true.',
        kubeconfig_invalid: 'The target file is not a valid Kubernetes v1 kubeconfig.',
        kubeconfig_multiple_paths: 'KUBECONFIG contains multiple paths. Choose one explicitly with destination=<path>.',
        kubeconfig_path_invalid: 'Choose a valid kubeconfig path.',
        kubeconfig_path_unavailable: 'The kubeconfig directory is not safely writable.',
        kubeconfig_response_invalid: 'The Luna server returned an invalid one-time kubeconfig. The issued credential was revoked when possible.',
        kubeconfig_target_changed: 'The kubeconfig changed while this command was running. No local changes were written.',
        kubeconfig_target_exists: 'The destination already exists. Use kubeconfig merge to preserve existing entries.',
        kubeconfig_target_unsafe: 'The target must be a regular file and cannot be a symbolic link.',
        kubeconfig_too_large: 'The kubeconfig exceeds the supported file size limit.',
        kubeconfig_write_failed: 'The kubeconfig could not be written safely.',
        kubeconfig_write_failed_revoke_failed: 'The kubeconfig write and automatic credential revocation both failed. Revoke the reported credential ID manually.',
  ...
  'zh-CN': {
    translation: {
      ...
      parameters: {
        ...
        category: '按命令分类筛选。',
        context: '使用稳定的 projectId:runtimeClusterId[:applicationId] 选择上下文；多个上下文可重复传入。',
        credential: '已保存的凭据名称。',
        credentialName: '短期 Kube Credential 的显示名称。',
        cursor: '上一页响应返回的分页游标。',
        destination: '目标 kubeconfig 路径；write 要求新文件，merge 会保留现有条目。',
        expiresInDays: '凭据有效天数，只允许 1、7 或 30。',
        kubeScope: 'kubectl 能力：read、write 或 connect；多个能力可重复传入。',
        ...
        query: '自由文本搜索条件。',
        replaceConflicts: '仅替换名称相同但内容不同的 cluster、user 和 context 条目。',
        risk: '按命令风险级别筛选。',
      ...
      commands: {
        ...
        kubeconfig: {
          merge: { summary: '创建短期凭据并安全合并到一个 kubeconfig。' },
          write: { summary: '创建短期凭据并安全写入新的 kubeconfig。' },
        },
        project: {
      ...
      errors: {
        ...
        dry_run_unsupported: 'kubeconfig 命令不支持 dry-run。',
        invalid_arguments: '输入参数校验失败。',
        kubeconfig_conflict: '目标 kubeconfig 存在同名冲突条目；请先核对，或显式设置 replaceConflicts=true。',
        kubeconfig_invalid: '目标文件不是有效的 Kubernetes v1 kubeconfig。',
        kubeconfig_multiple_paths: 'KUBECONFIG 包含多个路径，请使用 destination=<path> 明确选择一个。',
        kubeconfig_path_invalid: '请选择有效的 kubeconfig 路径。',
        kubeconfig_path_unavailable: '无法安全写入 kubeconfig 所在目录。',
        kubeconfig_response_invalid: 'Luna 服务返回的一次性 kubeconfig 无效；CLI 会尽力吊销已签发凭据。',
        kubeconfig_target_changed: '命令执行期间 kubeconfig 已变化，本次没有写入本地变更。',
        kubeconfig_target_exists: '目标文件已存在；请使用 kubeconfig merge 保留现有条目。',
        kubeconfig_target_unsafe: '目标必须是普通文件，不能是符号链接。',
        kubeconfig_too_large: 'kubeconfig 超出支持的文件大小上限。',
        kubeconfig_write_failed: '无法安全写入 kubeconfig。',
        kubeconfig_write_failed_revoke_failed: 'kubeconfig 写入与自动吊销凭据均失败，请按返回的 Credential ID 手动吊销。',
