export const resources = {
  'en-US': {
    translation: {
      common: {
        empty: 'No items found.',
        error: 'Command failed.',
        warning: 'Warning',
      },
      cli: {
        description: 'Luna DevOps command-line client for people and agents',
      },
      help: {
        headings: {
          usage: 'Usage:',
          arguments: 'Arguments:',
          options: 'Options:',
          globalOptions: 'Global Options:',
          commands: 'Commands:',
        },
        options: {
          version: 'Show the CLI version',
          help: 'Show command help',
          server: 'Override the Luna server origin',
          project: 'Select a project by immutable ID or identifier',
          output: 'Output format',
          lang: 'Output and help language',
          noColor: 'Disable terminal colors',
          noInteractive: 'Disable prompts',
          yes: 'Approve supported confirmation prompts',
          quiet: 'Suppress informational diagnostics',
          agent: 'Enable strict machine-readable agent mode',
          dryRun: 'Preview without applying',
          timeout: 'Request timeout, such as 30s or 2m',
          debug: 'Enable debug diagnostics',
          requestId: 'Use a request correlation ID',
          idempotencyKey: 'Use an idempotency key',
          insecureTls: 'Disable TLS verification when supported',
        },
        businessArguments: 'Business parameters in key=value form',
        canonicalCommand: 'Canonical command',
        quickStart: {
          title: 'Quick start:',
          login: '1. Sign in with OAuth Device Code:',
          customServer: '2. Sign in to another server when needed:',
          discover: '3. Discover commands:',
        },
        input: {
          title: 'Input syntax:',
          keyValue: 'Business parameters use key=value.',
          files: 'Use key=@file.json or key=@- for files, JSON, and multiline input.',
          repeat: 'Repeat a key for array parameters: key=first key=second.',
        },
        catalogSummary: '{{commands}} commands in {{categories}} categories. Use output=json for machine-readable results.',
        localeHint: 'Set LUNA_LANG=zh-CN or pass --lang zh-CN to change the help language.',
        categoryDescription: '{{category}} commands',
        categoryUsage: {
          title: 'Category usage:',
          catalog: 'Machine-readable discovery:',
        },
        details: {
          title: 'Command details:',
          command: 'Command',
          risk: 'Risk',
          project: 'Project selection',
          transport: 'Transport',
          endpoint: 'Endpoint',
          requiredScopes: 'Required scopes',
        },
        values: {
          risk: {
            low: 'low',
            medium: 'medium',
            high: 'high',
            critical: 'critical',
          },
          projectContext: {
            required: 'required',
            optional: 'optional',
            none: 'not used',
          },
          transport: {
            local: 'local',
            http: 'HTTP',
            sse: 'SSE',
            websocket: 'WebSocket',
            download: 'download',
            upload: 'upload',
          },
        },
        parameters: {
          title: 'Parameters:',
          none: 'No business parameters.',
          required: 'required',
          optional: 'optional',
          sources: 'sources',
          repeated: 'repeatable',
          sensitive: 'sensitive',
        },
        examples: {
          title: 'Examples:',
          fileHint: '# Replace large JSON or multiline values with parameter=@file.json.',
        },
        machineContract: 'Complete machine-readable contract:',
        errors: {
          unknownCommand: 'Unknown command',
          didYouMean: 'Did you mean',
          invalidArguments: 'Invalid command arguments.',
          nextStep: 'Next step',
        },
      },
      shortcuts: {
        login: 'Sign in to a Luna DevOps instance',
        logout: 'Remove the active Luna credential',
        whoami: 'Show the current authenticated identity',
        doctor: 'Check CLI, authentication, and server compatibility',
      },
      categories: {
        'access-token': 'Personal access tokens',
        'api': 'Diagnostic API requests',
        'application': 'Applications',
        'auth': 'Authentication and local credentials',
        'build': 'Builds and build templates',
        'completion': 'Shell completion scripts',
        'config': 'Project configuration and secrets',
        'dashboard': 'Dashboard summaries',
        'deployment': 'Deployment targets',
        'git': 'Git providers, repositories, and credentials',
        'health': 'Service health',
        'help': 'Command discovery and machine-readable contracts',
        'project': 'Project spaces and the default project',
        'release': 'Releases',
        'registry': 'Container registries, credentials, and images',
        'retention': 'Retention policies and cleanup',
        'runtime': 'Runtime resources, logs, and terminal operations',
        'user': 'Users and account settings',
        'version': 'CLI version and runtime information',
        'volume': 'Project volumes and archive transfer',
        'volume-transfer': 'Project volume transfer history',
      },
      parameters: {
        all: 'Apply to or include all matching items.',
        body: 'Request body; use @file or @- for structured or multiline data.',
        category: 'Filter by command category.',
        credential: 'Saved credential name.',
        cursor: 'Pagination cursor returned by the previous request.',
        expiresInDays: 'Credential lifetime in days; allowed values are defined by the command schema.',
        language: 'CLI language, such as zh-CN or en-US.',
        limit: 'Maximum number of results to return.',
        method: 'HTTP request method.',
        mode: 'Authentication mode; defaults to device-code. Use access-token explicitly for PAT fallback.',
        name: 'Resource name.',
        newName: 'New resource name.',
        page: 'Page number, starting from 1.',
        pageSize: 'Number of items per page.',
        path: 'Canonical command path or relative Luna API path.',
        project: 'Project ID or identifier selected for this command.',
        projectId: 'Project ID or identifier; identifiers are resolved to an immutable ID before the request.',
        projectID: 'Project ID or identifier; identifiers are resolved to an immutable ID before the request.',
        applicationId: 'Application ID or project-scoped identifier; identifiers are resolved before the request.',
        oauthApplicationId: 'OAuth application ID.',
        targetId: 'Deployment target ID or application-scoped stage; stages are resolved before the request.',
        deploymentTargetId: 'Deployment target ID or application-scoped stage; stages are resolved before the request.',
        releaseId: 'Immutable release ID.',
        container: 'Container name; omit it when the workload has only one container.',
        projectIdentifier: 'Human-readable project identifier.',
        projectName: 'Project display name.',
        query: 'Free-text search query.',
        risk: 'Filter by command risk level.',
        sortBy: 'Field used to sort the result.',
        sortOrder: 'Sort direction: asc or desc.',
        status: 'Resource status used as a filter or update value.',
        token: 'Access token for mode=access-token; read it from @file, @-, or LUNA_TOKEN.',
        transport: 'Filter by command transport.',
      },
      commands: {
        api: {
          request: { summary: 'Send a diagnostic request to a Luna API path.' },
        },
        auth: {
          login: { summary: 'Sign in with OAuth Device Code, or explicitly fall back to an access token.' },
          logout: { summary: 'Revoke OAuth tokens when possible and remove the local credential.' },
          refresh: { summary: 'Force one OAuth credential refresh for recovery or diagnostics.' },
          status: { summary: 'Refresh OAuth when needed and show authentication status safely.' },
        },
        completion: {
          bash: { summary: 'Generate Bash completion for Luna CLI.' },
          fish: { summary: 'Generate Fish completion for Luna CLI.' },
          powershell: { summary: 'Generate PowerShell completion for Luna CLI.' },
          zsh: { summary: 'Generate Zsh completion for Luna CLI.' },
        },
        help: {
          catalog: { summary: 'List commands from the machine-readable command catalog.' },
          command: { summary: 'Show the complete machine-readable contract for one command.' },
        },
        health: {
          doctor: { summary: 'Check local CLI, authentication, and server compatibility.' },
        },
        project: {
          create: { summary: 'Create a project space.' },
          current: { summary: 'Show the default project.' },
          delete: { summary: 'Delete a project space.' },
          get: { summary: 'Get one project space.' },
          list: { summary: 'List project spaces.' },
          unset: { summary: 'Clear the default project.' },
          update: { summary: 'Update a project space.' },
          use: { summary: 'Set the default project after server validation.' },
        },
        application: {
          create: { summary: 'Create an application.' },
          delete: { summary: 'Delete an application.' },
          get: { summary: 'Get one application.' },
          list: { summary: 'List applications.' },
          update: { summary: 'Update an application.' },
        },
        deployment: {
          create: { summary: 'Create a deployment target.' },
          delete: { summary: 'Delete a deployment target.' },
          exec: { summary: 'Open an interactive exec session for a deployment target.' },
          list: { summary: 'List deployment targets.' },
          update: { summary: 'Update a deployment target.' },
        },
        release: {
          create: { summary: 'Create a release.' },
          exec: { summary: 'Open an interactive exec session for a release.' },
          get: { summary: 'Get one release.' },
          list: { summary: 'List releases.' },
        },
        version: {
          show: { summary: 'Show Luna CLI version and runtime information.' },
        },
        volume: {
          adopt: { summary: 'Reference or adopt an existing persistent volume claim.' },
          delete: { summary: 'Delete a managed volume or detach a referenced volume.' },
          export: { summary: 'Export a project volume to a local archive with one complete stream.' },
          import: { summary: 'Import a local archive into a project volume.' },
          retry: { summary: 'Retry the latest failed volume operation.' },
          update: { summary: 'Rename or expand a project volume.' },
        },
        volumeTransfer: {
          retry: { summary: 'Retry a failed volume transfer with direction-specific authorization.' },
        },
      },
      volume: {
        progress: {
          upload: 'Uploading',
          export: 'Exporting',
          download: 'Downloading',
        },
      },
      confirm: {
        execute: 'Run this command?',
      },
      auth: {
        deviceCode: {
          code: 'Enter this device code to authorize Luna CLI:',
          browserOpened: 'A browser was opened. Complete authorization there; manual URL:',
          browserFallback: 'Open this URL in a browser to complete authorization:',
        },
      },
      errors: {
        command_required: 'Choose a command. Run luna --help to see available commands.',
        client_dry_run_resource_id_required: 'Client dry-run does not make lookup requests. Pass immutable resource IDs or remove --dry-run client.',
        auth_context_changed: 'The active server or login changed while the command was running. Review it and run the command again.',
        invalid_arguments: 'Input validation failed.',
        unauthenticated: 'Authentication is required.',
        oauth_access_denied: 'OAuth authorization was denied.',
        oauth_device_code_expired: 'The OAuth device code expired before authorization completed.',
        oauth_expired_token: 'The OAuth device code or token has expired.',
        oauth_invalid_client: 'This Luna CLI OAuth client is not accepted by the server.',
        oauth_invalid_grant: 'The OAuth grant or refresh token is invalid. Sign in again.',
        oauth_invalid_request: 'The OAuth server rejected the request as invalid.',
        oauth_invalid_scope: 'The OAuth server rejected the requested scopes.',
        oauth_invalid_token: 'The OAuth token is invalid. Sign in again.',
        oauth_network_error: 'The OAuth server could not be reached. Check the connection and try again.',
        oauth_redirect_refused: 'The OAuth server returned a redirect, so the CLI refused to forward credentials.',
        oauth_refresh_lock_timeout: 'Timed out waiting for another OAuth credential refresh to finish. Try again.',
        oauth_refresh_reauthentication_required: 'The previous OAuth refresh did not complete safely. Sign in again before continuing.',
        oauth_refresh_timeout: 'The OAuth credential refresh timed out. Sign in again before continuing.',
        oauth_refresh_token_required: 'The stored OAuth credential has no refresh token. Sign in again.',
        oauth_refresh_unavailable: 'The active credential cannot be refreshed automatically.',
        oauth_refresh_unsupported: 'This CLI runtime cannot refresh OAuth credentials.',
        oauth_request_replay_required: 'OAuth credentials were refreshed, but this request must be checked and run again manually.',
        oauth_request_failed: 'The OAuth request failed.',
        oauth_response_invalid: 'The OAuth server returned an invalid response.',
        oauth_revoke_timeout: 'The OAuth credential revocation timed out. Local sign-out will still continue.',
        oauth_server_error: 'The OAuth server could not complete the request. Try again later.',
        oauth_temporarily_unavailable: 'The OAuth server is temporarily unavailable. Try again later.',
        oauth_unauthorized_client: 'This Luna CLI OAuth client is not authorized for the requested flow.',
        oauth_unsupported_grant_type: 'The OAuth server does not support this login or refresh flow.',
        forbidden: 'You do not have permission to perform this operation.',
        not_found: 'The requested resource was not found.',
        resource: {
          not_found: 'The requested resource was not found. Check its parent scope, then use luna project list, luna application list, luna deployment list, or luna release list to find the immutable ID.',
        },
        resource_id_invalid: 'The resource ID is invalid. Expected prj_, app_, dplt_, or rel_ followed by 24 lowercase hexadecimal characters.',
        resource_id_required: 'releaseId must be an immutable rel_ ID. Run luna release list with the projectId to find it; use luna deployment exec for the deployment target\'s current container.',
        resource_reference_ambiguous: 'The resource reference matched more than one item. Pass an immutable ID.',
        resource_reference_invalid: 'The resource reference is invalid. Use a project/application identifier, deployment stage, or an ID beginning with prj_, app_, dplt_, or rel_.',
        resource_reference_not_found: 'No exact resource match was found. Check the parent scope, then run luna project list, luna application list, or luna deployment list to find its immutable ID.',
        resource_reference_scope_invalid: 'The parent resource ID required to resolve this reference is invalid.',
        resource_reference_scope_required: 'A parent resource is required to resolve this reference.',
        resource_reference_type_mismatch: 'The supplied ID belongs to a different resource type. Project, application, deployment target, and release IDs begin with prj_, app_, dplt_, and rel_ respectively.',
        resource_resolution_invalid: 'The server returned an invalid resource ID while resolving the reference.',
        resource_resolution_unsupported: 'This CLI runtime cannot resolve human-readable resource references.',
        stable_resource_id_required: 'Agent mode requires immutable resource IDs for resource parameters.',
        conflict: 'The resource changed or is not in the required state.',
        retry_later: 'The operation cannot be completed yet. Try again later.',
        service_failure: 'The service is temporarily unavailable.',
        confirmation_required: 'This operation requires confirmation.',
        operation_cancelled: 'Operation cancelled.',
        terminal_agent_unsupported: 'Interactive terminal sessions are not available in agent mode.',
        terminal_authorization_operation_missing: 'The terminal authorization contract is incomplete.',
        terminal_connection_closed: 'The terminal connection closed unexpectedly.',
        terminal_connection_error: 'The terminal connection failed.',
        terminal_connection_failed: 'The terminal connection could not be established.',
        terminal_connection_timeout: 'The terminal connection timed out.',
        terminal_exit_status_missing: 'The terminal closed without a remote exit status.',
        terminal_initialization_failed: 'The local terminal could not enter interactive mode.',
        terminal_input_failed: 'Terminal input could not be sent.',
        terminal_interrupted: 'The terminal session was interrupted.',
        terminal_output_backpressure_unsupported: 'The local output stream cannot safely receive more terminal data.',
        terminal_output_failed: 'Terminal output could not be written.',
        terminal_protocol_error: 'The terminal server sent an invalid protocol frame.',
        terminal_raw_mode_unsupported: 'The local terminal does not support raw mode.',
        terminal_remote_exit: 'The remote shell exited with a non-zero status.',
        terminal_resize_failed: 'The terminal size could not be synchronized.',
        terminal_tty_required: 'This command requires an interactive TTY.',
        server_dry_run_protocol_unsupported: 'Server dry-run is not supported for protocol commands. Use client dry-run to preview locally.',
        volume_transfer: {
          checksum_mismatch: 'The archive checksum does not match.',
          download_unauthorized: 'The download authorization expired or is invalid.',
          manifest_invalid: 'The block-volume export manifest is invalid.',
          manifest_mismatch: 'The block-volume export manifest does not match the transfer.',
          wait_timeout: 'Timed out waiting for the volume transfer to become ready.',
        },
      },
      table: {
        name: 'Name',
        status: 'Status',
        type: 'Type',
        createdAt: 'Created',
        updatedAt: 'Updated',
      },
    },
  },
  'zh-CN': {
    translation: {
      common: {
        empty: '没有找到数据。',
        error: '命令执行失败。',
        warning: '警告',
      },
      cli: {
        description: '面向用户和智能体的 Luna DevOps 命令行客户端',
      },
      help: {
        headings: {
          usage: '用法：',
          arguments: '参数：',
          options: '选项：',
          globalOptions: '全局选项：',
          commands: '命令：',
        },
        options: {
          version: '显示 CLI 版本',
          help: '显示命令帮助',
          server: '临时覆盖 Luna 服务地址',
          project: '使用不可变 ID 或标识符选择项目空间',
          output: '输出格式',
          lang: '输出和帮助语言',
          noColor: '关闭终端颜色',
          noInteractive: '关闭交互式提示',
          yes: '确认支持自动确认的操作',
          quiet: '隐藏非必要诊断信息',
          agent: '启用严格的机器可读智能体模式',
          dryRun: '仅预览而不实际应用',
          timeout: '请求超时，例如 30s 或 2m',
          debug: '显示调试诊断信息',
          requestId: '指定请求关联 ID',
          idempotencyKey: '指定幂等键',
          insecureTls: '在支持时关闭 TLS 证书校验',
        },
        businessArguments: '使用 key=value 形式传入业务参数',
        canonicalCommand: '标准命令',
        quickStart: {
          title: '快速开始：',
          login: '1. 使用 OAuth 设备码登录：',
          customServer: '2. 需要时登录其他服务：',
          discover: '3. 查找可用命令：',
        },
        input: {
          title: '输入规则：',
          keyValue: '业务参数统一使用 key=value。',
          files: '文件、JSON 和多行内容使用 key=@file.json 或 key=@- 从标准输入读取。',
          repeat: '数组参数重复传入同一个键，例如 key=first key=second。',
        },
        catalogSummary: '当前共有 {{categories}} 个分类、{{commands}} 条命令；使用 output=json 可获得机器可读结果。',
        localeHint: '可设置 LUNA_LANG=zh-CN，或传入 --lang zh-CN 切换帮助语言。',
        categoryDescription: '{{category}} 分类命令',
        categoryUsage: {
          title: '分类用法：',
          catalog: '机器可读的命令发现：',
        },
        details: {
          title: '命令详情：',
          command: '命令',
          risk: '风险级别',
          project: '项目空间选择',
          transport: '传输方式',
          endpoint: '接口',
          requiredScopes: '所需权限范围',
        },
        values: {
          risk: {
            low: '低',
            medium: '中',
            high: '高',
            critical: '严重',
          },
          projectContext: {
            required: '必需',
            optional: '可选',
            none: '不使用',
          },
          transport: {
            local: '本地',
            http: 'HTTP',
            sse: 'SSE',
            websocket: 'WebSocket',
            download: '下载',
            upload: '上传',
          },
        },
        parameters: {
          title: '业务参数：',
          none: '无需业务参数。',
          required: '必填',
          optional: '可选',
          sources: '输入来源',
          repeated: '可重复',
          sensitive: '敏感',
        },
        examples: {
          title: '示例：',
          fileHint: '# 较大的 JSON 或多行内容请改用 parameter=@file.json。',
        },
        machineContract: '完整机器可读命令契约：',
        errors: {
          unknownCommand: '未知命令',
          didYouMean: '你是否想输入',
          invalidArguments: '命令参数无效。',
          nextStep: '下一步',
        },
      },
      shortcuts: {
        login: '登录 Luna DevOps 实例',
        logout: '移除当前 Luna 登录凭据',
        whoami: '查看当前登录身份',
        doctor: '检查 CLI、认证与服务端兼容性',
      },
      categories: {
        'access-token': '个人访问令牌',
        'api': '诊断 API 请求',
        'application': '应用',
        'auth': '登录认证与本地凭据',
        'build': '构建与构建模板',
        'completion': 'Shell 自动补全脚本',
        'config': '项目配置与密钥',
        'dashboard': '看板摘要',
        'deployment': '部署目标',
        'git': 'Git 提供方、仓库与凭据',
        'health': '服务健康状态',
        'help': '命令发现与机器可读契约',
        'project': '项目空间与默认项目空间',
        'release': '发布',
        'registry': '镜像站、凭据与镜像',
        'retention': '数据保留策略与清理',
        'runtime': '运行资源、日志与终端操作',
        'user': '用户与账号设置',
        'version': 'CLI 版本与运行环境',
        'volume': '项目空间数据卷与归档传输',
        'volume-transfer': '数据卷传输记录',
      },
      parameters: {
        all: '对全部匹配项生效，或包含全部匹配项。',
        body: '请求体；结构化或多行内容请使用 @file 或 @-。',
        category: '按命令分类筛选。',
        credential: '已保存的凭据名称。',
        cursor: '上一页响应返回的分页游标。',
        expiresInDays: '凭据有效天数；可用值以命令 Schema 为准。',
        language: 'CLI 语言，例如 zh-CN 或 en-US。',
        limit: '最多返回多少条结果。',
        method: 'HTTP 请求方法。',
        mode: '认证方式；默认使用 device-code，仅在兜底时显式使用 access-token。',
        name: '资源名称。',
        newName: '新的资源名称。',
        page: '页码，从 1 开始。',
        pageSize: '每页返回的数据条数。',
        path: '标准命令路径或 Luna API 相对路径。',
        project: '本次命令使用的项目空间 ID 或标识符。',
        projectId: '项目空间 ID 或标识符；标识符会在请求前解析为不可变 ID。',
        projectID: '项目空间 ID 或标识符；标识符会在请求前解析为不可变 ID。',
        applicationId: '应用 ID 或项目空间内唯一标识符；标识符会在请求前完成解析。',
        oauthApplicationId: 'OAuth 应用 ID。',
        targetId: '部署目标 ID 或应用内唯一环境 stage；stage 会在请求前完成解析。',
        deploymentTargetId: '部署目标 ID 或应用内唯一环境 stage；stage 会在请求前完成解析。',
        releaseId: '不可变的发布 ID。',
        container: '容器名称；工作负载只有一个容器时可以省略。',
        projectIdentifier: '便于识别的项目空间标识。',
        projectName: '项目空间显示名称。',
        query: '自由文本搜索条件。',
        risk: '按命令风险级别筛选。',
        sortBy: '结果排序字段。',
        sortOrder: '排序方向：asc 或 desc。',
        status: '用于筛选或更新的资源状态。',
        token: 'mode=access-token 使用的访问令牌；从 @file、@- 或 LUNA_TOKEN 读取。',
        transport: '按命令传输方式筛选。',
      },
      commands: {
        api: {
          request: { summary: '向 Luna API 相对路径发送诊断请求。' },
        },
        auth: {
          login: { summary: '使用 OAuth 设备码登录，或显式回退到访问令牌。' },
          logout: { summary: '尽力吊销 OAuth 令牌并移除本地登录凭据。' },
          refresh: { summary: '强制刷新一次 OAuth 凭据，用于恢复或诊断。' },
          status: { summary: '在需要时自动刷新 OAuth，并安全查看认证状态。' },
        },
        completion: {
          bash: { summary: '生成 Luna CLI 的 Bash 自动补全脚本。' },
          fish: { summary: '生成 Luna CLI 的 Fish 自动补全脚本。' },
          powershell: { summary: '生成 Luna CLI 的 PowerShell 自动补全脚本。' },
          zsh: { summary: '生成 Luna CLI 的 Zsh 自动补全脚本。' },
        },
        help: {
          catalog: { summary: '列出机器可读命令目录中的命令。' },
          command: { summary: '查看一条命令的完整机器可读契约。' },
        },
        health: {
          doctor: { summary: '检查本地 CLI、认证与服务端兼容性。' },
        },
        project: {
          create: { summary: '创建项目空间。' },
          current: { summary: '查看默认项目空间。' },
          delete: { summary: '删除项目空间。' },
          get: { summary: '查看单个项目空间。' },
          list: { summary: '列出项目空间。' },
          unset: { summary: '清除默认项目空间。' },
          update: { summary: '更新项目空间。' },
          use: { summary: '经服务端校验后设置当前项目空间。' },
        },
        application: {
          create: { summary: '创建应用。' },
          delete: { summary: '删除应用。' },
          get: { summary: '查看单个应用。' },
          list: { summary: '列出应用。' },
          update: { summary: '更新应用。' },
        },
        deployment: {
          create: { summary: '创建部署目标。' },
          delete: { summary: '删除部署目标。' },
          exec: { summary: '进入部署目标的交互式执行会话。' },
          list: { summary: '列出部署目标。' },
          update: { summary: '更新部署目标。' },
        },
        release: {
          create: { summary: '创建发布。' },
          exec: { summary: '进入 Release 的交互式执行会话。' },
          get: { summary: '查看单个发布。' },
          list: { summary: '列出发布。' },
        },
        version: {
          show: { summary: '查看 Luna CLI 版本和运行环境。' },
        },
        volume: {
          adopt: { summary: '引用或纳管已有的持久卷声明。' },
          delete: { summary: '删除托管数据卷或解除外部数据卷引用。' },
          export: { summary: '以单次完整流将项目空间数据卷导出到本地归档。' },
          import: { summary: '以单次完整流将本地归档导入为项目空间数据卷。' },
          retry: { summary: '重试最近一次失败的数据卷操作。' },
          update: { summary: '重命名或扩容项目空间数据卷。' },
        },
        volumeTransfer: {
          retry: { summary: '按传输方向重新校验权限并重试失败的数据卷传输。' },
        },
      },
      volume: {
        progress: {
          upload: '正在上传',
          export: '正在导出',
          download: '正在下载',
        },
      },
      confirm: {
        execute: '确认执行此命令吗？',
      },
      auth: {
        deviceCode: {
          code: '请使用此设备码授权 Luna CLI：',
          browserOpened: '已尝试打开浏览器，请在页面中完成授权；手动访问地址：',
          browserFallback: '请在浏览器中打开此地址并完成授权：',
        },
      },
      errors: {
        command_required: '请选择要执行的命令。运行 luna --help 查看可用命令。',
        client_dry_run_resource_id_required: '客户端预演不会发送资源查询；请传入不可变资源 ID，或移除 --dry-run client。',
        auth_context_changed: '命令执行期间当前服务或登录身份发生了变化，请确认后重新执行。',
        invalid_arguments: '输入参数校验失败。',
        unauthenticated: '需要先完成身份验证。',
        oauth_access_denied: 'OAuth 授权已被拒绝。',
        oauth_device_code_expired: 'OAuth 设备码在授权完成前已过期。',
        oauth_expired_token: 'OAuth 设备码或令牌已过期。',
        oauth_invalid_client: '服务端不接受当前 Luna CLI OAuth 客户端。',
        oauth_invalid_grant: 'OAuth 授权或刷新令牌无效，请重新登录。',
        oauth_invalid_request: 'OAuth 服务拒绝了无效请求。',
        oauth_invalid_scope: 'OAuth 服务拒绝了请求的权限范围。',
        oauth_invalid_token: 'OAuth 令牌无效，请重新登录。',
        oauth_network_error: '无法连接 OAuth 服务，请检查网络后重试。',
        oauth_redirect_refused: 'OAuth 服务返回了重定向，CLI 已拒绝转发凭据。',
        oauth_refresh_lock_timeout: '等待另一个 OAuth 凭据刷新完成时超时，请重试。',
        oauth_refresh_reauthentication_required: '上一次 OAuth 刷新未能安全完成，请重新登录后再继续。',
        oauth_refresh_timeout: 'OAuth 凭据刷新超时，请重新登录后再继续。',
        oauth_refresh_token_required: '保存的 OAuth 凭据没有刷新令牌，请重新登录。',
        oauth_refresh_unavailable: '当前凭据无法自动刷新。',
        oauth_refresh_unsupported: '当前 CLI 运行环境不支持刷新 OAuth 凭据。',
        oauth_request_replay_required: 'OAuth 凭据已刷新，但此请求需要确认状态后手动重新执行。',
        oauth_request_failed: 'OAuth 请求失败。',
        oauth_response_invalid: 'OAuth 服务返回了无效响应。',
        oauth_revoke_timeout: 'OAuth 凭据吊销超时，本地退出仍会继续。',
        oauth_server_error: 'OAuth 服务无法完成请求，请稍后重试。',
        oauth_temporarily_unavailable: 'OAuth 服务暂时不可用，请稍后重试。',
        oauth_unauthorized_client: '当前 Luna CLI OAuth 客户端无权使用所请求的流程。',
        oauth_unsupported_grant_type: 'OAuth 服务不支持当前登录或刷新流程。',
        forbidden: '当前账号没有执行此操作的权限。',
        not_found: '未找到请求的资源。',
        resource: {
          not_found: '未找到请求的资源。请先检查父级范围，再通过 luna project list、luna application list、luna deployment list 或 luna release list 查找不可变 ID。',
        },
        resource_id_invalid: '资源 ID 格式无效；应为 prj_、app_、dplt_ 或 rel_ 前缀，加 24 位小写十六进制字符。',
        resource_id_required: 'releaseId 必须是 rel_ 开头的不可变 ID；请携带 projectId 执行 luna release list 查找。若要进入部署目标的当前容器，请使用 luna deployment exec。',
        resource_reference_ambiguous: '资源引用匹配到多个结果，请改用不可变 ID。',
        resource_reference_invalid: '资源引用无效；请使用项目/应用标识符、部署环境 stage，或 prj_、app_、dplt_、rel_ 开头的 ID。',
        resource_reference_not_found: '没有找到精确匹配的资源。请检查父级范围，再通过 luna project list、luna application list 或 luna deployment list 查找不可变 ID。',
        resource_reference_scope_invalid: '解析资源引用所需的父资源 ID 无效。',
        resource_reference_scope_required: '解析资源引用需要先提供父资源。',
        resource_reference_type_mismatch: '给定 ID 属于其他资源类型；项目、应用、部署目标和 Release ID 应分别以 prj_、app_、dplt_、rel_ 开头。',
        resource_resolution_invalid: '服务端在解析资源引用时返回了无效资源 ID。',
        resource_resolution_unsupported: '当前 CLI 运行环境不支持解析可读资源引用。',
        stable_resource_id_required: '智能体模式下的资源参数必须使用不可变资源 ID。',
        conflict: '资源已发生变化或当前状态不允许此操作。',
        retry_later: '当前暂时无法完成操作，请稍后重试。',
        service_failure: '服务暂时不可用。',
        confirmation_required: '此操作需要先确认。',
        operation_cancelled: '操作已取消。',
        terminal_agent_unsupported: '智能体模式不能打开交互式终端会话。',
        terminal_authorization_operation_missing: '终端授权契约不完整。',
        terminal_connection_closed: '终端连接意外关闭。',
        terminal_connection_error: '终端连接发生故障。',
        terminal_connection_failed: '无法建立终端连接。',
        terminal_connection_timeout: '建立终端连接超时。',
        terminal_exit_status_missing: '终端关闭时没有返回远端退出状态。',
        terminal_initialization_failed: '本地终端无法进入交互模式。',
        terminal_input_failed: '无法发送终端输入。',
        terminal_interrupted: '终端会话已中断。',
        terminal_output_backpressure_unsupported: '本地输出流无法安全接收更多终端数据。',
        terminal_output_failed: '无法写入终端输出。',
        terminal_protocol_error: '终端服务返回了无效协议帧。',
        terminal_raw_mode_unsupported: '本地终端不支持原始输入模式。',
        terminal_remote_exit: '远端 Shell 以非零状态退出。',
        terminal_resize_failed: '无法同步终端窗口尺寸。',
        terminal_tty_required: '此命令必须在交互式 TTY 中运行。',
        server_dry_run_protocol_unsupported: '协议命令不支持服务端预演；请使用客户端预演在本地查看计划。',
        volume_transfer: {
          checksum_mismatch: '归档校验和不一致。',
          download_unauthorized: '下载授权已过期或无效。',
          manifest_invalid: '块卷导出 manifest 无效。',
          manifest_mismatch: '块卷导出 manifest 与传输记录不匹配。',
          wait_timeout: '等待数据卷传输就绪超时。',
        },
      },
      table: {
        name: '名称',
        status: '状态',
        type: '类型',
        createdAt: '创建时间',
        updatedAt: '更新时间',
      },
    },
  },
} as const

export type SupportedLocale = keyof typeof resources
export const SUPPORTED_LOCALES = Object.freeze(Object.keys(resources) as SupportedLocale[])
